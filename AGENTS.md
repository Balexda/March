...
- **Keep subsystem contract docs current at edit time.** When a change alters a subsystem's public surface, update that subsystem's `contract.md` in the same change — contract docs are maintained as part of normal editing (the Smithy tools used for most edits already update affected docs), and the mechanically-derivable regions are refreshed by a **deterministic** extractor (the planned `npm run docs:contracts:extract`, from Fastify endpoints / exported TS signatures). There is **no per-PR CI or AI freshness gate**; the planned `npm run docs:contracts:check` is an opt-in, advisory local check. This is the maintenance convention defined by Feature 6 of the contract-documentation track — see [`docs/rfcs/2026-002-layered-testing-framework/02-subsystem-contract-documentation-track.features.md`](docs/rfcs/2026-002-layered-testing-framework/02-subsystem-contract-documentation-track.features.md).
