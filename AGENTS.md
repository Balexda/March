...
- `src/bootstrap/`: install/update/manifest/skill deployment (CLI-installation bootstrap) and `march update`. The bootstrap is run as the first-run step of `march init` (folded in, not a separate command). The top-level `march init [profile]` is wired in `src/cli/program.ts` over `src/legate/`: with no profile it just runs that first-run bootstrap; with a `<profile>` it also onboards the profile.
...
- **Keep subsystem contract docs current at edit time.** When a change alters a subsystem's mapped public surface, update that subsystem's `contract.md` in the same change; when a change touches no mapped public surface, no contract update is required. Treat this as normal edit-time upkeep for authors and Smithy agents (the Smithy tools used for most edits already update affected docs), not as a separate freshness pass. Feature 7's mechanically-derivable regions are refreshed by a **deterministic** extractor (the planned `npm run docs:contracts:extract`, from Fastify endpoints / exported TS signatures). There is **no per-PR CI or AI freshness gate**; Feature 5's `npm run docs:contracts:check` is an opt-in, advisory local check that never blocks a PR, slice, or merge. This convention follows March's autonomous-component posture in [`docs/vision.md`](docs/vision.md) and [`docs/operating-philosophy.md`](docs/operating-philosophy.md), and is defined by Feature 6 of the contract-documentation track — see [`docs/rfcs/2026-002-layered-testing-framework/02-subsystem-contract-documentation-track.features.md`](docs/rfcs/2026-002-layered-testing-framework/02-subsystem-contract-documentation-track.features.md).
...
- **Keep observability in lock-step with the dispatch machinery.** When you add a lifecycle action or a new dispatch path, emit a span for it; when you add a failure mode, emit an *errored* span so it surfaces in traces; when a new process joins a trace, reuse the deterministic id helpers (kept byte-for-byte identical across `src/observability/trace-ids.ts`, `src/legate/init.ts`, and `src/observability/in-spawn-emitter.ts`). **Each slice is one trace** keyed by its id: a service-side span that observes or acts on a slice (Herald's `herald.pr.*` via `src/observability/herald-trace.ts`, brood teardown, Castra ops) nests on the deterministic slice id as a **child** rather than starting its own root, so `legate.dispatch` stays the originator. Debugging a stuck task is reading that one trace for the absent/errored/wrong-attribute leg — and a trace that can't locate the stall is itself a telemetry gap to close. New metrics/labels go in `src/observability/spawn-metrics.ts` (spawns) or `src/observability/hatchery-metrics.ts` (the Hatchery service) — low-cardinality only, never per-spawn/slice ids or concrete request paths; new logs go through the pino logger in `src/observability/logger.ts`; then update `docker/grafana/dashboards/` to match. The full guide is [`docs/Observability.md`](docs/Observability.md).
