...
- **Keep subsystem contract docs current at edit time.** When a change alters a subsystem's mapped public surface, update that subsystem's `contract.md` in the same change; when a change touches no mapped public surface, no contract update is required. Treat this as normal edit-time upkeep for authors and Smithy agents (the Smithy tools used for most edits already update affected docs), not as a separate freshness pass. The mechanically-derivable regions are refreshed by a **deterministic** extractor (the planned `npm run docs:contracts:extract`, from Fastify endpoints / exported TS signatures). There is **no per-PR CI or AI freshness gate**; the planned `npm run docs:contracts:check` is an opt-in, advisory local check. This convention follows March's autonomous-component posture in [`docs/vision.md`](docs/vision.md) and [`docs/operating-philosophy.md`](docs/operating-philosophy.md), and is defined by Feature 6 of the contract-documentation track — see [`docs/rfcs/2026-002-layered-testing-framework/02-subsystem-contract-documentation-track.features.md`](docs/rfcs/2026-002-layered-testing-framework/02-subsystem-contract-documentation-track.features.md).
