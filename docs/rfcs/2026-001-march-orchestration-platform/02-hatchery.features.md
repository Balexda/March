...
Hatchery's role in the system, per [`docs/operating-philosophy.md`](../../operating-philosophy.md), is to eliminate operator intervention in *setting up to run the steps* — the operator does not assemble container args, juggle credentials, or hand-tune security posture per session. Profiles declare configuration once and every consuming role (spawn, steward, legate, future) reads from the same declarative source. Each feature in this map should serve that role; the operator's job is to author profile YAML in a text editor, not to thread arguments through CLI invocations.
...
> **Architecture note (container-service split, 2026-05).** Since this map was
> drafted, Hatchery runs as a **containerized Fastify service** (`march hatchery
> serve` executes the spawn-orchestration flow; `march hatchery spawn` is a thin
> HTTP client). The profile schema/validation work (F1–F6) is linked *into* that
> service — validation runs server-side, and the `march hatchery` CLI verbs are
> HTTP clients. The Hatchery drives the **Steward** session through the **Castra**
> HTTP API (it no longer mounts `agent-deck`) and registers each spawn with
> **Brood** at launch. Where this map references `.agent-deck/conductor/...`
> directly, read it as "the Steward session hosted in Castra"; the mini-legate
> directory is retained only as the *origin pattern*, not a live integration path.

## Features

### Feature 1: Profile Schema and Validation Library

**Description**: Define the declarative Hatchery profile data model and ship a pure, side-effect-free validator. The schema structurally enforces the Appendix A threat-model constraints — bind-mount rejection (A2), env whitelist as the only env mechanism (A3), snapshot include/exclude (A5), and per-backend network endpoint allowlist (A7) — so unsafe configurations are impossible by construction rather than caught at runtime. The schema covers every field required by the M2 success criteria: base image, file mounts, tool availability, network access, and security posture. Every field currently on M1's `SpawnConfig` interface (`capDrop`, `user`, `networkMode`, `memoryLimit`, `cpuLimit`, `timeoutSeconds`, `envWhitelist`) maps 1:1 onto the profile schema's security-posture fields so the M1 refactor in F5 is a pure rename, not a redesign — the additional schema fields (`baseImage`, `fileMounts`, `tools`, etc.) extend the surface for non-spawn profiles without altering the M1 mapping.
...
**Scope Boundaries**:
- Includes: profile data model (TypeScript types + on-disk schema), required `baseImage` field (container image reference), required `fileMounts` field as a structured closed-set list (never raw host paths — A2 forbids them; permitted shapes are named volumes and snapshot-derived read-only mounts), structural enforcement of A2/A3/A5/A7 (no host-path entries accepted by `fileMounts`; `envWhitelist` as the *only* env mechanism; `snapshot.include`/`snapshot.exclude` fields; `network.allowlist` for per-backend endpoints), 1:1 field mapping from M1 `SpawnConfig` to the profile schema's security-posture fields, `version` and `name` identity fields, optional `tools?: { allowed?: string[]; disallowed?: string[] }` field shipped but unconsumed in M2, pure-function validator producing typed errors, profile-fixture test set (valid + invalid examples) that doubles as schema documentation
...
**User-Facing Value**: The operator has a single, predictable place where profiles live, and any tool or skill that needs a profile by name can resolve it consistently. Profiles can be version-controlled by copying them out of the user directory into a repo if desired, but M2 does not introduce repo-local profile overrides.
...
**Description**: Extend M1's `march init` to seed the profile directory with two opinionated default Hatchery profiles, materializing them as editable YAML files so the operator sees exactly the defaults they will run. The first seeded profile (`spawn`) encodes M1's container/security values from `SPAWN_CONFIG` (`src/spawn-config.ts`) verbatim into the profile's container/security fields, **plus** M1's snapshot patterns from `SNAPSHOT_EXCLUSION_PATTERNS` (`src/snapshot.ts`) verbatim into the profile's `snapshot.exclude` field, plus the `ANTHROPIC_API_KEY` envWhitelist — so F5's M1 refactor is a regression-free swap. The second seeded profile is `pr-management`: a container profile for the **Steward** — a Claude Code session, hosted in **Castra** and driven via the Castra HTTP API, that handles PR-management workflows (issue triage, branch creation, PR open/update, review reply) by running in auto-mode steered by F6 skills, following the mini-legate origin pattern (`.agent-deck/conductor/legate-march/`). No separate PR-management agent component is built — the workflow is `pr-management` profile + auto-mode + skills, hosted in Castra, full stop. Re-init is idempotent: existing profile files are not overwritten unless the operator passes `--force`.
...
**Scope Boundaries**:
- Includes: extension of `march init` to write the two seeded YAML profiles to `~/.march/profiles/`, byte-faithful encoding of M1 `SPAWN_CONFIG` (`src/spawn-config.ts`) into the `spawn` profile's container/security fields, byte-faithful encoding of M1 `SNAPSHOT_EXCLUSION_PATTERNS` (`src/snapshot.ts`) into the `spawn` profile's `snapshot.exclude` field, the `ANTHROPIC_API_KEY` envWhitelist on `spawn`, a `pr-management` profile with the broader posture an auto-mode Claude Code session needs to operate on a repo and call the GitHub API (e.g. `GH_TOKEN`/`GITHUB_TOKEN` in the envWhitelist, `network.allowlist` extended to include `api.github.com`, repo-snapshot file mounts — exact field values land in F3's spec phase), idempotent seeding (skip-if-exists by default, `--force` flag to overwrite), inline doc comments inside each materialized YAML file
...
**Description**: Add Hatchery-flavored skills to the March CLI's deployed skill catalog, following the SmithyCLI pattern established by M1 Feature 1. Skills cover the operator workflows that depend on the new Hatchery surface: authoring a new profile, validating one before dispatch, choosing a profile for a spawn, and understanding how the schema's structural constraints map to the Appendix A threat model. F6 also ships the **PR-management skill set** that steers the Steward — a Claude Code session hosted in Castra running under the `pr-management` profile (issue triage, branch creation, PR open/update, review reply, merge readiness) — modeled on the mini-legate skill set (origin pattern: `.agent-deck/conductor/legate-march/`), where auto-mode + scoped skills compose into an operator workflow without bespoke orchestrator code.
...
**Scope Boundaries**:
- Includes: new skill markdown files for Hatchery workflows (profile authoring, validation, selection, threat-model mapping), PR-management skill set for Claude Code auto-mode running under the `pr-management` profile (mini-legate pattern), deployment via the existing `march init` skill-deployment path established in M1 Feature 1
...
|----|-------------|-----------------|--------|------------|--------|------------|
| SD-001 | Identity of the second seeded profile in F3: a speculative `pr-management` schema-only stub (matches RFC narrative, not exercised by any M2 runtime consumer) vs. a runnable example like `skill-author` or `doc-edit` (M2 could actually invoke it via spawn dispatch). The choice shapes F3's payload and the M2 demo surface. Recommended lean: `pr-management` stub for narrative consistency, with the runnable example deferred until M3+ has a consumer that can exercise it. | clarify:Feature Boundaries / Scope Within the Milestone | Critical | Medium | resolved | Operator decision (PR #97 review): the second seeded profile is `pr-management`, and it **is** runnable — its consumer is the Steward — a Claude Code session hosted in Castra running in auto-mode steered by F6 skills (mini-legate origin pattern: `.agent-deck/conductor/legate-march/`). F3 ships the container profile; F6 ships the PR-management skill set; no separate agent component is needed. |
...
| Milestone 3: Brood | depended upon by | When M3 introduces additional containerized session types beyond `spawn` and `pr-management`, those components consume Hatchery profiles via F1's schema and F2's loader. The `pr-management` profile already ships in M2 (F3) and is exercised by Claude Code in auto-mode + F6 skills (mini-legate pattern), so no separate PR-management component is needed in M3. |
| Milestone 4: Herald | depended upon by | Herald now runs as a containerized observation service and, like other containerized components, consumes a Hatchery profile via the same F1/F2 surface. M2 imposes no constraint on Herald's event-sourcing model. |
| Milestone 5: Legate | depended upon by | Legate already runs as a container service (`src/hatchery/legate-container.ts` ships an ad-hoc profile today); the `legate` Hatchery profile formalizes that current container config via the F1/F2 surface. M2 did not yet ship a declarative `legate` profile. |
