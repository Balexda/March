...
**Statio** — The forge (`gh`) gateway. The forge counterpart to Castra: Castra put the one `agent-deck` install behind an HTTP API; Statio puts the one `gh` install behind one. A containerized Fastify service (one shared `march-statio` host on the `march` network) that owns the *mechanics* of talking to the forge — PR/CI/review reads now (`repoInfo`, `listPrs`, `getPr`, `reviewThreads`), broader issue/comment/PR-mutation CRUD later — behind a swappable `ForgeClient` interface and a bearer-token HTTP API. Scope is **`gh` only**; `git` stays a direct local operation owned by the services that run it. The ownership split is: forge **mechanics** → Statio (stateless; no decisions, no state); forge **observation** → Herald, via Statio; forge **decisions/mutations** → Legate, via Statio; Hatchery owns **no** forge logic. Draft spec at `specs/2026-05-26-006-statio-forge-gateway/`; origin epic Balexda/March #250, motivated by the #245 self-heal ownership question and paired with the local-only Hatchery fast-path that landed in #249.
...
| 6 | Statio forge (`gh`) gateway | Direct `gh` usage in Herald `src/observe/sense-io.ts`, the legate loop image, and `legate.unwedge`'s `clean-stale-branch.sh`; `gh` install + `~/.config/gh` mount duplicated across consumer images | Stand up Statio (forge counterpart to Castra) behind a typed `ForgeClient` + bearer-token HTTP API; v1 reads (`repoInfo`/`listPrs`/`getPr`/`reviewThreads`), v2 mutations later. Herald and legate cutovers (dropping `gh` from each image) are follow-on PRs once the service ships. Spec drafted at `specs/2026-05-26-006-statio-forge-gateway/`. Origin epic Balexda/March #250 (`area:statio`); pairs with the local-only Hatchery fast-path that landed in #249. |
