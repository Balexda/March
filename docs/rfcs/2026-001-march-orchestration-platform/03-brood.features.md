...
| Milestone 2: Hatchery | depends on (loose) | F1 tolerates SpawnRecords both with and without an M2 F5 `profile` field — M3 begins safely even if M2 F5 has not yet landed. F6's skill catalog accumulates onto the M2 F6 skill set; deployment plumbing is unchanged. |
