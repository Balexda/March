...
- **PR gate**: $0, deterministic only — includes L0, L1, and cassette-replayed L2/L3. No live backend spawns. Wall-clock budget is *minutes, not tens of minutes*, and is bounded by the slowest staged gate (see below). This is where the cassette pivot pays off: integration coverage at unit-test cost.
- **Staged workflows**: PR-gate runs as separate jobs per scope, not one monolithic suite. **L0 runs first as the fail-fast gate**; on L0 pass, **L1 + cassette-replayed L2 + cassette-replayed L3 run in parallel**. The split keeps the feedback loop tight on broken-fundamentals failures — a change that violates an L0 invariant should fail in seconds, not after the full L3 suite finishes — while giving the integration tier the wall-clock it actually needs. The exact gating pattern (L0-gates-rest vs. L0+L1-gates-L2/L3) is a knob to tune once we see real CI timings; the principle is fast feedback on cheap failures, parallelism on the rest.
...
- **Phase 1 — Make today's tests legible.** Add scope/determinism tags to each existing test file (the gap-analysis table above is the seed). Define four scripts — `test:l0`, `test:l1`, `test:l2-cassette`, `test:l3-cassette` — and wire the CI workflow as **staged jobs**: L0 first as the fail-fast gate, the rest in parallel on L0 pass (per the §6 staged-workflows policy). Keep `npm test` as a convenience alias that runs the same set locally, and `npm run test:slow` as the local opt-in for real-Docker + cassette-recording mode. The four-script split also lets Phase 1.5 / Phase 2 add their scripts without re-plumbing the workflow each time. Update `CONTRIBUTING.md` to match. Cheap; unlocks the rest.
