...
Legate is started through the `march legate` command group. `march legate init` registers a deployment profile for a repository and manager context, including the repository path, profile name, worker group, paired Legate steward/conductor metadata, local March CLI path, service endpoints, and merge policy. `march legate serve` starts the profile-agnostic service process; the back-compat `march legate loop` alias reaches the same serve entrypoint. `march legate recover <sliceId>` is an operator-visible recovery request path that appends an event for the running service to consume on its next tick.
...
| Invalid profile or manager metadata | `march legate init` fails fast for invalid names or missing local prerequisites; the service tick skips an invalid profile and emits a diagnostic rather than dispatching with ambiguous manager context. |
