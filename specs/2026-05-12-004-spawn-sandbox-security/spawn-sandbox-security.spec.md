...
2. **Given** the private spawn network, **When** the dispatch runs, **Then** an HTTP CONNECT proxy sidecar container is started on that network and given `--network bridge` for outbound access to the operator's network.
3. **Given** a running spawn, **When** the backend CLI inside the spawn container reads `HTTP_PROXY` and `HTTPS_PROXY` (or the equivalent backend-specific configuration), **Then** all outbound HTTPS requests are routed through the proxy sidecar.
...
- **FR-004**: At Stage 4, a private user-defined Docker network MUST be created per spawn (e.g., `march-spawn-net-<spawn-id>`). The spawn container and the proxy sidecar container MUST both be joined to this private network.
- **FR-005**: At Stage 4, an HTTP CONNECT proxy sidecar container MUST be launched on the private spawn network and given outbound network access. The sidecar MUST be configured with the selected backend's `allowedEgressHosts` as its hostname allowlist. The sidecar MUST refuse CONNECT requests for hosts NOT on the allowlist AND MUST refuse CONNECT-by-IP requests (where no hostname is available for matching).
- **FR-006**: The spawn container MUST receive `HTTP_PROXY` and `HTTPS_PROXY` environment variables pointing to the proxy sidecar's loopback endpoint on the private spawn network. (Pass-through form — value computed at launch time, not stored in `SpawnBackend.requiredEnvVars`.)
...
- **FR-009**: A new CLI verb `march spawn verify <spawn-id>` MUST be registered as a sibling of `march spawn dispatch`. It MUST emit a structured JSON report to stdout with one entry per Appendix A control (A1, A2, A3, A4, A5, A7, A8 — A6 omitted) containing fields `control`, `status` (`"pass"` | `"fail"` | `"n/a"`), `expected`, and `observed`. Exit codes: `0` (all pass), `1` (one or more fail), `2` (usage error).
