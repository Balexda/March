# Tasks: Profile Type Skeleton and Validator Surface

**Source**: `specs/2026-05-12-003-profile-schema-and-validation-library/profile-schema-and-validation-library.spec.md` — User Story 1
...
## Slice 1: Add Profile Module Surface
<!-- audience: builder; mode: how-to; length: 5-15 steps; diagram: optional; examples: forbidden -->

**Goal**: Deliver the first importable `src/profile/` library increment with the canonical public types and a total `validateProfile` entry point for non-object inputs.

**Justification**: This slice is a working increment because downstream TypeScript consumers can import the documented F1 surface, narrow on `ValidationResult.ok`, and call `validateProfile` on arbitrary non-object values without crashes. It intentionally leaves schema-field validation to later user stories.

**Addresses**: FR-001, FR-002, FR-003, FR-006; Acceptance Scenarios 1.1-1.3
...
- [ ] **Add the profile public type surface**
  ...
  - `Profile`, `ValidationResult`, `ValidationError`, and `ValidationErrorCode` are exported from `src/profile/index.ts`
  - The exported profile subtypes match the contract-level skeleton for later stories to fill with behavior
  - TypeScript consumers can use `Profile` in type positions through the public barrel
...
- [ ] **Implement non-object validation behavior**
  ...
  _Acceptance criteria:_
  - `validateProfile` is exported from `src/profile/index.ts`
  - `null`, primitives, and arrays return a failure result instead of throwing
  - Non-object failures use `WrongType` at the root JSON pointer
  - Returned validation errors include operator-readable messages
  - The implementation has no I/O, clock, randomness, environment, or mutable global dependency

**PR Outcome**: Merging this slice creates the canonical `src/profile/` public API and validates non-object root inputs without throwing, unblocking all later F1 validation stories.
...
|----|-------|------------|----------|
| S1 | Add Profile Module Surface | — | — |

### Cross-Story Dependencies

| Dependency | Direction | Notes |
|------------|-----------|-------|
| User Story 2: Identity, Version, and `baseImage` Validation | depended upon by | Uses the public module and validator result surface created by this story. |
| User Story 3: Container Security and Resources are 1:1 with M1 SpawnConfig | depended upon by | Extends the exported `Profile` subtypes and validator created by this story. |
| User Story 4: A3 Structural Enforcement — `envWhitelist` is the Only Env Mechanism | depended upon by | Requires the container subtype and validator entry point from this story. |
| User Story 5: A7 Network Policy is a Closed Allowlist with Structured Endpoints | depended upon by | Requires the network subtype and validator entry point from this story. |
| User Story 6: A2/A5 — File Mounts and Snapshot Policy Enforce Threat Model Structurally | depended upon by | Requires the mount and snapshot subtypes and validator entry point from this story. |
| User Story 7: Optional `tools` Policy, Shipped But Unconsumed | depended upon by | Requires the tools subtype and validator entry point from this story. |
| User Story 8: Profile Fixture Corpus as Schema Documentation | depended upon by | Exercises the public types and validation error codes built across this story and later validation stories. |
