# Tasks: Optional `tools` Policy, Shipped But Unconsumed

**Source**: `specs/2026-05-12-003-profile-schema-and-validation-library/profile-schema-and-validation-library.spec.md` — User Story 7
...
## Slice 1: Validate Optional Tools Policy
<!-- audience: builder; mode: how-to; length: 5-15 steps; diagram: optional; examples: forbidden -->

**Goal**: Extend `src/profile/` so the optional top-level `tools` policy is part of the public profile schema, is validated for structural shape and overlap, and remains unconsumed by M2 runtime dispatch.

**Justification**: This slice is a standalone increment because it delivers the complete M2 tools-policy contract in one PR: profile authors can declare future tool posture now, invalid policy shapes are rejected deterministically, and no runtime component starts enforcing a field that the spec says is shipped for M3+.

**Addresses**: FR-001, FR-002, FR-003, FR-004, FR-005, FR-006, FR-007, FR-023; Acceptance Scenarios 7.1-7.5
...
- [x] **Expose optional tools policy types**
  ...
  - `ToolsPolicy` remains exported through the profile barrel
  - `Profile["tools"]` is optional and assignable to the exported `ToolsPolicy`
  - `allowed` and `disallowed` entries remain string-array policy lists
  - Profiles without `tools` remain representable at the type level
...
- [x] **Validate tools policy structure**
  ...
  _Acceptance criteria:_
  - A missing top-level `tools` field validates successfully
  - Empty `tools` objects and empty inner policy arrays validate successfully
  - Non-object `tools` values produce structural validation errors at `/tools`
  - Non-array `allowed` or `disallowed` values produce structural validation errors at their field paths
  - Non-string tool entries produce `WrongType` at indexed JSON-pointer paths
...
- [x] **Reject overlapping tool policy entries**
  ...
  _Acceptance criteria:_
  - A tool name present in both policy lists produces `ToolOverlap` at `/tools`
  - The overlap error message includes the offending tool name
  - Non-overlapping `allowed` and `disallowed` lists validate successfully
  - Empty or absent inner policy lists do not produce overlap errors
  - Multiple tools-policy and structural errors remain deterministically ordered
  - The validator remains synchronous, pure, and non-throwing for malformed `tools` input

**PR Outcome**: Merging this slice makes `tools` a validated, optional M2 profile field with deterministic diagnostics while preserving the explicit boundary that no M2 runtime consumer enforces it.
...
|----|-------|------------|----------|
| S1 | Validate Optional Tools Policy | — | — |

### Cross-Story Dependencies

| Dependency | Direction | Notes |
|------------|-----------|-------|
| User Story 1: Profile Type Skeleton and Validator Surface | depends on | Uses the public profile module, validation result surface, root object validation behavior, and error ordering created by Story 1. |
| User Story 8: Profile Fixture Corpus as Schema Documentation | depended upon by | The final fixture corpus exercises absent, valid, and overlapping tools-policy examples introduced by this story. |
