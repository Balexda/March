# Tasks: A7 Network Policy is a Closed Allowlist with Structured Endpoints

**Source**: `specs/2026-05-12-003-profile-schema-and-validation-library/profile-schema-and-validation-library.spec.md` — User Story 5
...
## Slice 1: Validate Closed Network Policy
<!-- audience: builder; mode: how-to; length: 5-15 steps; diagram: optional; examples: forbidden -->

**Goal**: Extend `src/profile/` so `Profile.network` is validated as a closed discriminated union over `bridge`, `none`, and `allowlist`, with structured endpoint validation for allowlisted outbound access.

**Justification**: This slice is a standalone increment because operators get the complete A7 schema-level guarantee: every profile declares its network posture, unrestricted M1-compatible bridge mode remains explicit, no-network mode is representable, and allowlisted network access requires typed endpoints that F5 can translate without guessing.

**Addresses**: FR-005, FR-012, FR-013; Acceptance Scenarios 5.1-5.6
...
- [ ] **Prove the network policy type is a closed discriminator**
  ...
  - `NetworkPolicy` remains exported through the profile barrel
  - `Profile["network"]` is assignable to the exported `NetworkPolicy` subtype
  - Extracting `mode` values from `NetworkPolicy` yields only `"bridge"`, `"none"`, and `"allowlist"`
  - Extracting an `allowlist` field from the `bridge` and `none` variants resolves to `never`
  - The coverage remains compile-time only and adds no runtime behavior

- [ ] **Validate bridge and none network postures**

  Extend network validation so the non-allowlist discriminators satisfy AS 5.1 and AS 5.2 while preserving closed-world behavior from the earlier profile stories. These modes should accept only their discriminator field.

  _Acceptance criteria:_
  - `network: { mode: "bridge" }` validates without network-field errors
  - `network: { mode: "none" }` validates without network-field errors
  - A non-object `network` value produces `WrongType` at `/network`
  - A missing `network.mode` produces a discriminator-shaped validation error at `/network/mode`
  - Unknown fields under `bridge` and `none` modes are rejected with `UnknownField`

- [ ] **Require non-empty allowlists in allowlist mode**

  Extend network validation for `mode: "allowlist"` so the `allowlist` array is required and non-empty, and so discriminator-specific fields remain closed to their documented shape.

  _Acceptance criteria:_
  - `network: { mode: "allowlist", allowlist: [{ host: "api.anthropic.com", port: 443, protocol: "https" }] }` validates without network-field errors
  - Missing `allowlist` under allowlist mode produces `EmptyAllowlist` at `/network/allowlist`
  - An empty `allowlist` array produces `EmptyAllowlist` at `/network/allowlist`
  - `allowlist` under `bridge` or `none` mode produces `UnknownField` at `/network/allowlist`
  - Unknown fields on the allowlist network object are aggregated with endpoint-level errors

- [ ] **Validate structured network endpoints**

  Validate each `NetworkEndpoint` entry in allowlist mode using the host, port, and protocol rules from the data model. Keep endpoint validation structural and deterministic so F4 diagnostics can key off error codes and JSON-pointer paths.

  _Acceptance criteria:_
  - Valid hostname endpoints with `http`, `https`, or `tcp` protocols pass
  - IPv4 literal hosts pass according to the data-model resolution inherited from SD-002
  - URL-shaped hosts such as `http://foo` produce `InvalidHost` at the endpoint host path
  - Ports outside `1` through `65535`, non-integer ports, and non-number ports produce `InvalidPort` at the endpoint port path
  - Protocol values outside `"http"`, `"https"`, and `"tcp"` produce `InvalidProtocol` at the endpoint protocol path
  - Unknown fields inside endpoint objects produce `UnknownField` at their indexed JSON-pointer paths
  - Endpoint errors are deterministically ordered with all other profile validation errors

**PR Outcome**: Merging this slice makes A7 structurally enforceable through the profile schema: each profile must explicitly declare bridge, none, or allowlisted network posture, and allowlisted access is represented as typed endpoints for later Docker-layer enforcement.
...
|----|-------|------------|----------|
| S1 | Validate Closed Network Policy | — | — |

### Cross-Story Dependencies

| Dependency | Direction | Notes |
|------------|-----------|-------|
| User Story 1: Profile Type Skeleton and Validator Surface | depends on | Uses the public profile module, network subtype, and validator result surface created by Story 1. |
| User Story 3: Container Security and Resources are 1:1 with M1 SpawnConfig | depends on | Preserves the M1-compatible `bridge` posture that replaces legacy root `networkMode` while keeping the rest of the parity fields independent. |
| User Story 6: A2/A5 — File Mounts and Snapshot Policy Enforce Threat Model Structurally | depended upon by | Later mount and snapshot validation remains independent but relies on the same closed-world object-validation conventions this story extends. |
| User Story 8: Profile Fixture Corpus as Schema Documentation | depended upon by | The final fixture corpus exercises the network policy variants and endpoint error codes introduced by this story. |
