...
**Why this priority**: Gemini's introduction makes per-backend env validation a real requirement. F2 deferred this check to F4 (per the comment at `container-launch.ts:133–134`) because F2 only had one possible env var and the failure mode was discoverable. F3 has two possible env vars, so the same failure now becomes ambiguous to the operator unless caught up-front. F4 still owns network-policy and threat-model hardening; the env-presence check belongs to F3 because F3 introduces the divergence.

**Independent Test**: Unset `GEMINI_API_KEY` and run `march spawn dispatch --backend gemini --prompt-file <path>`. The command exits with `USAGE_ERROR` (2), the error message names the backend (`gemini`) and the missing variable (`GEMINI_API_KEY`), and no git or Docker call has been made (no worktree, no branch, no image, no container).

**Acceptance Scenarios**:

1. **Given** the operator runs `march spawn dispatch --backend gemini --prompt-file <path>` with `GEMINI_API_KEY` unset, **When** the dispatch action runs the auth pre-flight, **Then** the command exits with `USAGE_ERROR` (2), prints `"Backend 'gemini' requires GEMINI_API_KEY: missing GEMINI_API_KEY. Set the variable(s) and re-run."` (or equivalently structured), and no worktree, branch, image, or container has been created.
...
- **FR-014**: When the auth pre-flight detects any missing required env var, the command MUST exit with `USAGE_ERROR` (2), print a clear error naming the backend and each missing variable, and produce no worktree, branch, image, or container.
...
|--------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------|--------|------------|--------|------------|
| SD-001 | `envWhitelist` field removal vs. retention on `SpawnConfig` interface — FR-012 says drop it entirely, but US3 says "F2 tests pass unchanged." Removing the field will break any F2 unit test that asserts `SPAWN_CONFIG.envWhitelist === ["ANTHROPIC_API_KEY"]` (e.g., `src/spawn-config.test.ts:78`). The reconciled position is: remove the field and redirect F2 unit assertions to read from `claudeCodeBackend.requiredEnvVars`. End-to-end behavioral preservation holds; literal test-source preservation does not. | Domain & Data Model     | High   | Medium     | open   | —          |
...
- **SC-003**: With `GEMINI_API_KEY` unset, `march spawn dispatch --backend gemini --prompt-file <path>` exits with `USAGE_ERROR` (2), names the backend and the missing variable, and creates no worktree, branch, image, or container.
