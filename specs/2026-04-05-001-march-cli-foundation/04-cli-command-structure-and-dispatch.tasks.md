...
1. **Slice 1** — No dependencies. Pure test extension. Land first — the no-args content assertions act as a regression net for the production code change in Slice 3 (confirming the `!commandHandled` path change does not accidentally fire for valid no-args invocations).
2. **Slice 2** — No dependency on Slice 1; can be implemented in parallel. Resolves the spawn alignment conflict and locks the spawn command behavior before Slice 3 modifies the adjacent `!commandHandled` block.
