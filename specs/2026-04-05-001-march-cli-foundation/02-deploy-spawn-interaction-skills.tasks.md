...
**Justification**: Acceptance Scenarios 1 and 2 (placeholder files deployed; all manifest paths exist on disk) are already satisfied by User Story 1 Slice 3, which explicitly claimed them. The spec governs US2 as deploying placeholder files — the content is correct as-is and must not change here. AS3 ("skills are discoverable by the AI agent as commands or prompts") is the unique contribution of this story. Claude Code discovers commands by scanning `~/.claude/commands/*.md` using the filename minus `.md` as the slash command name; prompts work the same way in `~/.claude/prompts/`. The test on line 24 of `skills.test.ts` previously permitted arbitrary characters in filenames (`/^march\..*\.md$/`), which would have allowed names with spaces or uppercase letters that break Claude Code discovery. Line 70 previously only checked that `#` appears somewhere in content rather than that content opens with an H1. These two gaps are what this slice closes.
...
- [x] Run `npm test` and verify all tests pass, including the three per-skill title tests on lines 84–103 that assert placeholder content — those tests are correct for this milestone and must continue to pass.
