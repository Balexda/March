...
- This feature is a **written policy only**: a "Test Layer Migration" heading in `CONTRIBUTING.md` that enumerates verbatim the conditions under which the three pre-existing vitest L2 tests must be ported to Cucumber.js, plus the recorded starting state of those tests. It does **not** perform any port and does **not** introduce runtime code, a deferral-record system, or an audit command. [Critical Assumption]
- The policy is a **"Cucumber-on-first-material-change" trigger, not a preemptive port** (RFC §Design Considerations and §M1 criteria, line 145): the three tests stay in vitest until the next material change touches them, at which point the change PR ports the affected scenario to Cucumber.js. The actual port is M3-and-later work and is out of scope here.
- The three governed files are exactly `src/spawn/container-launch.test.ts`, `src/spawn/snapshot-build.test.ts`, and `src/hatchery/legate-container.test.ts` — the L2-shaped vitest files Feature 1 tagged `@l2 @deterministic @ci` in place.
- The corrected premise inherited from Feature 1 is that those three tests **mock `node:child_process` and exercise no real Docker** — earlier documentation mischaracterized them as exercising real Docker. The policy records this corrected starting state.
- "Material change" is scoped to **semantic** changes to a governed test — its assertions, the mocked process behavior it controls, its fixtures, the subsystem boundary it exercises, or the production behavior under test. Non-semantic edits (formatting, comments, import sorting, tag-block edits, mechanical renames that preserve the test contract) do **not** trigger a port. The conditions must be precise enough that PR authors and reviewers do not relitigate "material change" on every diff (RFC line 86).
...
2. **Given** the policy text, **When** it is read, **Then** it enumerates verbatim the conditions that make a change material — at minimum changes to assertions, mocked process behavior, fixtures, the exercised subsystem boundary, or the production behavior under test.
...
**Independent Test**: Read the policy and confirm it names the three governed files exactly and states that each is tagged `@l2 @deterministic @ci`, mocks `node:child_process`, exercises no real Docker, and remains in vitest until a material change.

**Acceptance Scenarios**:

1. **Given** the policy, **When** the governed set is read, **Then** it names exactly `src/spawn/container-launch.test.ts`, `src/spawn/snapshot-build.test.ts`, and `src/hatchery/legate-container.test.ts`.
2. **Given** the policy, **When** the starting state is read, **Then** it records that the three tests are tagged `@l2 @deterministic @ci` in place, mock `node:child_process`, and exercise no real Docker.
...
1. **Given** a PR that changes assertions, mocked command behavior, fixtures, or the production path exercised by a governed test, **When** the author applies the policy, **Then** the change is classified as material and a Cucumber.js port of the affected scenario is required by the policy.
...
- A change touches helper or fixture code imported by a governed test rather than the test file itself; the policy should make clear whether the trigger keys on the test file path or on the governed test's exercised behavior (recorded as SD-001).
...
- **FR-002**: The policy MUST name the governed tests exactly: `src/spawn/container-launch.test.ts`, `src/spawn/snapshot-build.test.ts`, and `src/hatchery/legate-container.test.ts`.
- **FR-003**: The policy MUST enumerate verbatim the conditions that make a change to a governed test material — at minimum changes to its assertions, mocked process behavior, fixtures, exercised subsystem boundary, or production behavior under test.
...
- **FR-007**: The policy MUST record the corrected starting state: the three tests are tagged `@l2 @deterministic @ci` in place, mock `node:child_process`, and exercise no real Docker.
...
- **Governed Legacy L2 Test**: One of the three child-process-mocked vitest files tagged `@l2 @deterministic @ci` by Feature 1 and subject to the migration trigger.
...
- Feature 1 has already tagged the three governed tests `@l2 @deterministic @ci` in place and corrected the "real Docker" mischaracterization in `CONTRIBUTING.md` and `docs/testing-strategy.md`; this feature builds on that corrected premise rather than re-establishing it.
...
|----|-------------|-----------------|--------|------------|--------|------------|
| SD-001 | Does the migration trigger key on edits to the governed test file itself, or on any change that alters the governed test's exercised behavior (including edits to helpers, fixtures, or production code it covers)? The verbatim conditions must pick one to stay unambiguous on a PR. | clarify:Edge Cases | Medium | Low | open | — |

## Out of Scope

- The actual Cucumber.js port of any of the three governed tests (triggered on first material change; the porting infrastructure is M3 and later).
- The Feature 1 day-one tag disposition of the three tests and the broader `CONTRIBUTING.md` tag/script/quarantine references owned by Features 1–3.
...
- **SC-001**: A contributor can locate the "Test Layer Migration" policy in `CONTRIBUTING.md` and identify the three governed tests from it.
