...
| `src/worktree.test.ts` | Spawn worktree creation + rollback |
| `src/snapshot.test.ts` | Snapshot Exclusion List, build-context assembly |
| `src/snapshot-build.test.ts` | Spawn Dockerfile generation, image build, image removal |
| `src/spawn-record.test.ts` | SpawnRecord write / state transitions / rollback |
