...
Requires Node 20 or 22. `march spawn dispatch` additionally requires `git` and `docker` on `PATH`. `march legate init` requires `git`, [`agent-deck`](https://github.com/asheshgoplani/agent-deck), and Python 3.9+ for agent-deck's conductor bridge; `march legate init --with-container` also requires Docker.
...
| `march legate init` | Set up a per-repo *mini-legate* conductor (Smithy plan→PR→fix loop) on top of [agent-deck](https://github.com/asheshgoplani/agent-deck). Pass `--with-container` to also build and launch the Hatchery-managed Legate container. |
