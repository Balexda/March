...
describe("legate module", () => {
  ...
  describe("initLegate", () => {
    ...
    it("stages a deterministic loop scaffold by default", async () => {
      ...
      expect(result.loopStagingDir).toBe(
        path.join(home, ".march", "legate", "march-legate-agent", "loop"),
      );
      expect(result.loopConductorDir).toBe(
        path.join(home, ".agent-deck", "conductor", "march-legate-loop"),
      );
      expect(result.loopSetupRan).toBe(false);
      expect(result.loopConfigured).toBe(false);

      // The loop runs as the `march legate loop` service container; init only
      // stages the meta sidecar it mounts. The legacy legate-loop.mjs is gone.
      const metaPath = path.join(result.loopStagingDir!, "legate-loop-meta.json");
      expect(
        fs.existsSync(path.join(result.loopStagingDir!, "legate-loop.mjs")),
      ).toBe(false);
    ...
    it("keeps processor names valid for long custom conductor names", async () => {
      ...
      expect(result.loopConductorDir).toContain(result.processorName!);
    });
    ...
    it("can disable processor staging with processor=false", async () => {
      ...
      expect(result.loopConductorDir).toBeUndefined();
      expect(
