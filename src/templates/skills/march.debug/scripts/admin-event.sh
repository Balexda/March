...
# Optional:
#   MARCH_CLI   path to the march executable (default baked per context).
#
...
if [ -z "${MARCH_HERALD_URL:-}" ]; then
  echo "march.debug: MARCH_HERALD_URL is not set — point it at Herald (e.g. $HERALD_DEFAULT_URL)." >&2
  exit "$EX_USAGE"
fi
...
CLI="${MARCH_CLI:-$MARCH_CLI_DEFAULT}"
