...
Stage transitions into `merged` are written by the deterministic loop: directly after its own successful gated auto-merge of a `pr-open` slice, or when it observes `state == "MERGED"` on a PR the operator merged in the GitHub UI. Both paths are equivalent from the loop's terminal cleanup perspective.
