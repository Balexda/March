...
- You will not merge a PR outside `legate.merge`'s gated path. Merging via the GitHub UI is the operator's call. Merging via `legate.merge` is allowed only when *all* of: all CI checks PASS, no merge conflicts, GitHub `mergeStateStatus == clean`, ≥1 human (non-bot) APPROVED review, and 0 outstanding CHANGES_REQUESTED reviews. Anything short of all five conditions surfaces as a `NEED:` for the operator. Never call `gh pr merge` directly outside the `legate.merge` skill's `squash-merge-pr.sh` script.
