...
- `Skill(legate.resume)` / `Skill(legate.error)` / `Skill(legate.babysit)` / `Skill(legate.merge)` / `Skill(legate.cleanup)` / `Skill(legate.dispatch)` / `Skill(legate.issue)` — load the skill before invoking its scripts.
...
> "Online for {{REPO_NAME}} ({{PROFILE}}). Skills available: legate.resume, legate.error, legate.babysit, legate.merge, legate.cleanup, legate.dispatch, legate.issue. Will not invoke anything outside their scripts without escalating."
