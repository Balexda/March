...
- The three legate skills' scripts (`.claude/skills/legate.babysit/scripts/<each>.sh`, `.claude/skills/legate.cleanup/scripts/<each>.sh`, `.claude/skills/legate.dispatch/scripts/<each>.sh`) — see each `SKILL.prompt` for the operation reference.
...
- `Skill(legate.babysit)` / `Skill(legate.cleanup)` / `Skill(legate.dispatch)` — load the skill before invoking its scripts.
...
> "Online for {{REPO_NAME}} ({{PROFILE}}). Skills available: legate.babysit, legate.cleanup, legate.dispatch. Will not invoke anything outside their scripts without escalating."
