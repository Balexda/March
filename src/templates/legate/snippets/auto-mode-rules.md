...
- `Skill(legate.resume)` / `Skill(legate.error)` / `Skill(legate.babysit)` / `Skill(legate.merge)` / `Skill(legate.cleanup)` / `Skill(legate.issue)` — load the skill before invoking its scripts.
...
- **Any** invocation of `python` or `python3` — inline `-c`, script file, or as a pipe target. Filtering or reshaping JSON belongs in an audited skill script or the deterministic processor. If the operation you want isn't expressible in an existing skill script, the right move is to extend the relevant skill in the source template (`src/templates/legate/skills/<skill>/scripts/`) and re-run `march legate init` — never to reach for `python`.
...
> "Online for {{REPO_NAME}} ({{PROFILE}}). Skills available: legate.resume, legate.error, legate.babysit, legate.merge, legate.cleanup, legate.issue. Will not invoke anything outside their scripts without escalating."
