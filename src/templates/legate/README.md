...
**`legate.babysit`** — existing-PR mechanics. Loaded after resume on every heartbeat. Owns the decision tree (`MERGED → CONFLICTING → FAIL → unresolved threads → all-clear`), stage transitions, and every script that mutates an existing PR via the worker that owns it (clean rebase, conflict resolution, CI re-trigger, `/smithy.fix` dispatch, error-worker restart). The paired deterministic processor mirrors the low-judgement subset on its own tick: PR discovery, PR-state refresh, errored-worker restart, conflict prompts, review-thread `/smithy.fix`, CI `/smithy.fix`, and all-clear transitions. Ambiguous states are written to `processor-requests.ndjson` and announced to the Legate conductor with `[PROCESSOR]`.
