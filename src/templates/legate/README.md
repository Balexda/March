...
**Gated auto-squash-merge (deterministic loop, not a skill)** — the auto-merge gate lives in the legate *service*, which has Herald access and reads the profile's per-task-type merge policy. For each slice in `pr-open`, the loop applies the gate (PR `OPEN`, no conflicts, all CI PASS, no review threads owed, plus the human-review gates — ≥1 human approval and 0 outstanding CHANGES_REQUESTED — each *relaxable per task type* via the profile's merge policy, e.g. `cut` can drop the approval requirement). When the gate clears it runs `gh pr merge --squash --match-head-commit` (the SHA pin defeats a race against a worker push) and transitions the slice to `merged`. A gate the loop can't clear leaves the PR for the operator; cleanup archives terminal PRs and tears down their worker sessions on the next tick.
...
```
              ...
              └──────────────────┬───────────────────┘  ──────────▶  MERGED  ─▶  **ARCHIVED**
                                                          (loop auto-merges      (loop removes
                                                           per the merge policy   worker + worktree;
                                                           OR operator merges     slice moves into
                                                           in UI)                 archived_slices)
                                 │
...
- **Merges go through the gate.** The deterministic legate service squash-merges a `pr-open` slice only when *all* of: the PR is `OPEN`, has no merge conflicts (`mergeable == MERGEABLE`), all CI checks pass, no review threads owed, plus the human-review gates — ≥1 human (non-bot) APPROVED review and 0 outstanding CHANGES_REQUESTED. The two human-review gates are *relaxable per task type* by the profile's merge policy (e.g. `cut` can drop the approval requirement for AI-authored planning artifacts); CI/conflicts/threads are never relaxed. A gate the service can't clear leaves the PR for the operator.
